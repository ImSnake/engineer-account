<template>
  <div  class="authWrap bg bg-primary">
    <form @keyup.enter="$emit('logEvent', { login: login, password: password })"  class="authFormWrap">
          ...
          <span class="elz d-flex grPin grY2 a-X borB2 evNone">
            ...
            <span class="elz p-rel growZ d-flex a-PR">
              <span :class="login ? loginProps.tooltip : ''" class="elz p-rel d-flex a-X r2 cur-help opHovOut">
                <span class="elz fn11 bold al-right pH8 op0 opHovIn10 trns2 trnsVis oH ellipsis">{{ login ? loginProps.text : '' }}</span>
      ...
      <div class="elz authFormShow d-block h80">
        <label class="elz d-grid grPos fn fnLInvD fn-primary-t fnL20 fnHovL10 fnFow-focus fnFowL0">
          <input @keyup="validationPassword" v-model="password" :class="password ? passwordProps.classApproval : ''" ref="password" type="password" placeholder="Пароль"                  class="elz d-block grPin grY2 w100p borB2 h48 pH32 ellipsis trns2 invPssSib br brLInvD br-primary brL-10 brHovL-20 brFoc-focus brFocL0 fn fn-primary-t" />
          <span class="elz d-flex grPin grY2 a-X borB2 evNone">
            <span class="elz p-rel d-flex a-X s24 trns2">
            ...
            <span class="elz p-rel growZ d-flex a-PR">
              <span :class="password ? passwordProps.tooltip : ''" class="elz p-rel d-flex a-X r2 cur-help opHovOut">
                <span class="elz fn11 bold al-right pH8 op0 opHovIn10 trns2 trnsVis oH ellipsis">{{ password ? passwordProps.text : '' }}</span>
                <span class="elz p-rel d-flex a-X s24 r2 evAuto">
      ...
      <div class="elz authFormShow d-block h48">
        <div @click="$emit('logEvent', { login: login, password: password })" :class="dataIsValid ? '' : 'uDisabled'" class="mV16 elz buttonTest d-flex a-X al-center selNone r3 h48 w100p fn14 cur-pointer bg bg-ok bgHovL3 bgActL-10 bgSelL-10 br brFD brLF-10 br-ok fn fn-ok-t fnHovL10 fnSelL10 trnsStyle trns">
          Авторизация
        </div>
      </div>
